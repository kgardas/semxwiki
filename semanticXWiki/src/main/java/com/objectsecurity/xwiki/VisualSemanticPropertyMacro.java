...
@Component("visSemProp")
public class VisualSemanticPropertyMacro extends AbstractMacro<VisualSemanticPropertyMacroParameters>{

	public VisualSemanticPropertyMacro() {
		super("visSemProp", DESCRIPTION, VisualSemanticPropertyMacroParameters.class);
		System.err.println();
		System.err.println();
		System.err.println("Visual semantic property macro is being initialized...");
		System.err.println();
	}

	@Override
	public List<Block> execute(VisualSemanticPropertyMacroParameters arg0,
		...
		try {
			...
			if (linkAttr != null && !linkAttr.equals("")) {
				resLink = ctx.getProperty(name, property_prefix, linkAttr);
			}
			...
			l.add(new WordBlock("<set vis property operation aborded!>"));
			return l;
		}
		ArrayList<Block> l = new ArrayList<Block>();
		if (!links) {
	...
	private static final String DESCRIPTION = "Visual semantic property macro";
